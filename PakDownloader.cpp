...
UAsyncPakDownloader* UAsyncPakDownloader::Download(const FString &URL, const FString &SavePath)
{
...
void UAsyncPakDownloader::HandleDownloadProgress(FHttpRequestPtr InRequest, uint64 bytesSent, uint64 bytesReceived)
{
	...
	if (InRequest.IsValid())
	{
		//总大小
		//int32 TotalSize = 0;
		//if (InRequest->GetResponse().IsValid())
		//{
		//	TotalSize = (int32)InRequest->GetResponse()->GetContentLength();
		//}

		//// (int32)bytesReceived：强转为int32适配你之前的蓝图引脚
		OnProgress.Broadcast(0, 0, SaveFilePath, (int32)bytesReceived);
